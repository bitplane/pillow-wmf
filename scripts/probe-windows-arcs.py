...
def main():
    with reference_surface(128, 128) as (gdi, dc, bits):
        ...
        for case in cases:
            ...
            if actual != expected:
                print("arc-control-difference", case, actual, expected)
            ...
            native_points = tuple((x, y) for x, y, _ in fixed_path_points(gdi, dc))
            native_path = DevicePath.polyline(native_points)
            assert native_path.vertices == DevicePath(cubics).vertices, (case, native_path.vertices)
