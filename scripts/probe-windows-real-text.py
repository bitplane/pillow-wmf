...
def main():
    ...
    tables = {}
    families = [family for _, family, _ in FONTS] + ([FAMILY] if args.layout else [])
    ...
    with private_fonts(paths):
        for name, family, recorder in layout_cases() if args.layout else cases():
            print(f"\n[{name}]", flush=True)
            source = recorder.to_bytes()
            size = LAYOUT_SIZE if args.layout else SIZE
            sample = b"A B A B" if args.layout else b" ".join(SAMPLES)
