...
path uses the [legacy GIQ membership rules](gdi-strokes.md#cosmetic-lines) for segment coverage and style steps. A path may exit and
...
Clipping regressions compare a small surface with a crop of a larger surface
across styles, background modes, transposed axes and reversed traversal.
Windows-generated PNGs establish the expected coverage and phase.
