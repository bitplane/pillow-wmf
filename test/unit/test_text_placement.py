...
def test_rotated_opaque_paired_output_is_explicitly_unsupported():
    dc = context()
    ...
    before = dc.image.tobytes(), dc._position, tuple(dc.calls)
    with pytest.raises(UnsupportedOperation, match="Rotated opaque"):
        dc.ext_text_out(0, 0, b"ABA", options=0x2000, advances=(19, 7, 23, -5, 17, 11))
    assert (dc.image.tobytes(), dc._position, tuple(dc.calls)) == before
